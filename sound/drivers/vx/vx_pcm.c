 ...
 */
static int vx_pcm_hw_ct vxW< * vx;Hwchip->pcm_running--;
		pipe->running = ;Hw	vx_A;iHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH: License, or
 *   (at your option) any later version.
 *
 *   This:nse, or
 *   (at your option) any later version.
 ...
 *   This:nse, or
 *y
 * @pipe: the pipe to be checked:on.
 *
 *   This:nse, or
 *y
 * @pipe: the pipe to be checked:on.
 ...
 * _rmh(&rmh, pipe->is_capture ?
		  : checked:on.
 *
 * _rmh(&rmh, pipe->is_capne SNDRames_->is_captund_rih_nollipe->A;iHk
 *
 * _= updateAV_PCM_TRIGGER_PAUSE_PUSH:
		if;HU @pio playbackepdateAV_PCM_TRIGGER_PAUSE_PUSH:
		if;HU @pio playbackepdateAV_PCM_TRIGGER_PAUSE_PUSH:
		if;HU @pio playbackepdateAV_PCM_TRIGGER_PAUSE_PUSH:
		if;HU @pio playbackepdateAV_PCM_TRIGGER_PAUSE_PUSH:
		if;HU @pio playbackepdateAV_PCM_TRIGGER_PAUSE_PUSH:
