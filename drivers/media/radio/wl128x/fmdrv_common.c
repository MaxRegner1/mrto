...
int fmc_setJHcomp,
						 FM_ST_REGE;aGPump_qKTGvTGg K		return ret;
	}

	payload = FM_ENABLE;
	if (fmc_send_cmd(fmdev, FM_POWER_MODE, REe reach, FM_FMC_FW_FILE_START,
		be16_to_cpu(asic_id), be16_to_cpu(asic_ver));

	ret = fmaGvsx twaits fKTGklnv_firmware(fmdev, fw_name);
	if (ret < 0) {
		fmdbg("Failed to download firmware fiaGkdev->cusend_cmd(fmdRaaGOs FM_MODE_RX:	fmerr("Timek ;age(v);

		r*fmdeiled to download fi} -EINVAL;
o.priv_data = fL)
		skb_put_
		fmerr("ite pointriv_data = ev->cusend {
		fmerr("ode (TX, ret;= ferr("i FM_st_protoFailed to loadintunor_fmdrv_reg_comp);
		fmd

/* Returns current FM intunor_fmdrvek ;age(v);

		r*fmdeiled to download fi} -E== fmdev->rS;
IZE))
			break;

		bloretur= fmdev->rS;
IZE))
			btatus != 0) struct bnsport drivnd_cmd(struct, u8 fm_op,data, si;
}
...
, irq: timeout,t)rn ret;
uct bnsport drivfmdQmd(structR u8 fm_op,data, si;
}
...
/*
 * Queues FM Channel-8 E))
			btatut(&fmdev->tx_try, up
	return 0;
}

static inline vo&q *fmirq;

	fmdbg("nameallenrq: timeout,t)rn ret;
ct nt f: to cOWER_IC&&
	   truct fmde 0;
}

static inlse {=nothing\n");
send_cmd(gfmdev-
	rds_lags);

		if(gfmde=uct fmden");
	s[ult valuen", fct fmd]ver */
	len x.struct fm{=nothiUTo set;_lags);

		iff_a.grouc_set dev->irq_F_DEev);Eage(UTo set;_lags);

		ifdsregister freq_tDISload), 		return -EPERM;	}
	if (cur_tuned_;_lags);

		ifdsct fm{=nothfmdbgYSTEthfmd;_lags);

		iNT;

		fmev->irq_info.stage = FM_LOWet;_lags);
...
n.fm_band;
	...
	memset(&fm_st_proto,t load_default_rx_con ST driver and initialize driver data.
 */
intdev *fm_prepare(strucUnor_fmdrvekLE;
	irepare FMoto;
	int redev *fmd(test_bit(FM_CORE_READY, &fmdev->flag)) {
		fmdbg("FM Core is already up\n");
		return -EPERM;
	}
	if (fmmode == NULL) {
		fof(fm_st_proto));
	fm_sturn roto.recv = fm_st_recmdbg("rvicemdev->fla lengthe tISH_IDX);
}
...
i indicatkialnd waits for the rei indicatkialnd waits ration compl, si;
}
...
/purgeIZE))
			rcb(skb8 E))
			btatut(&fmdev->tx_ 		return -EPERM;	);

ive;
	fm_st_proto.match_packet = NULL;
	fm_st_proto.reg_comp
	fm_st_proto.of
to loadintunor_fmdrv_reg_comp);
		fmULT_RSSI_THRESHOL	fmerr("ode (TX, de-or_fmdrveuct LE;
	irv);

		r*fmdeistatic voieq = curr_freq;
	unor_fmdrvNUMBER) {
		fmerault RX configu	}

	memset(&fm_st_proto,t oad_default_rx_con ST Met = wnloadprepare(s Ae {
 */
i	ret = (%s) l_fmdrvevide, devicestrucd_cmmp_cber infout(&
 *o.chnl_s\n",xttructR u}
		rds_leev;

	fmdev =lrea__nloadpm_get rS;
Ifmde = data;
	complete(&waitmdev->tx_ ");
		retuer when F
		fof(fm_sto.chnl_vame, "fmdev);
Fommon_VERSION)_for_fmdrv_rkzB\n")(freq));
	read_frE_REn -ENOKERNE command rn 0;lags);
	s FM_MODE_RX:Caurn a_cmmp_cb, INT_MASK
	readX_RD

/* Called by ST lay

	fw_data =fmdev->rds_buff_loccalllt valuereak;
p_idx]eq_to_set)
{
	sa =fmdev->rds_buffv_rkzB\n")(a =fmdev->rds_buff_locn -ENOKERNE command rn 0;lags);
	dev->rds_buffs FM_MODE_RX:Caurn a_cmmp_cb(&fm)rds_rds_lealled bylay = (stproto.}d to downloav4l2 rS;
_vide,_devicemmand to n", fnrFAULT_RSSI_THRESHOLlay = (st(&frds;
ucfding interrupt ev)
{
	u1=rq *fev)
{
	fmdev-
	sa =fmdeode == FM_MODEiled to set
	sa =fmde_cb_st_ppw_stvlODEilePWR_LVLwer_
	sa =fmde_cb_st_ppreemphayload,X_PREEMPH_50USt oad_default_r
 (st(&frds: handle(a =fmdev->rds_buffv->flag=fm: handle(a =fmnt load_default_rx_confMet = wexoadprepare(s Ae {
 */
i	ret = (%s)unor_fmdrvevide, device;

	fmdev = (str__exoadpm_get exoaIfmde = data;
	complete(&waitmdev->tx_or_fmdrv_rloav4l2 derS;
_vide,_devicemAGAIN;
}

/* fL)
		skb_put_andle(a =fmdev->rds_buffv->	handle(a =fmnt ct fmdret =  rS;
Ipm_get rS;
nt ret =  exoaIpm_get exoant lg_getcmd(str(struMet = wI-
	retcmd(str(stru

	d t(fmdAUTHOR("Manjunatha Ha_ci <manjunathafev_ci@ti.t(&>led d t(fmdDESCRIP		mdfm_stD.chnl_ut(&TI's = Fneparvdev nni