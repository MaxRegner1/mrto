...
int mmdvfs_qos_ut_set(const char *val, const struct kernel_param *kp)
{
	...
	case 6:
		...
		/* No		/* MaIC\n");
		/* No		/yd>SCOMP_DEFAULT);
		mm_qto notifier lock_cam_cI*	mmyv.-set_requeam_cI*	mmyv.-setyd>bAL_C(req->mastf (lyd>klengt@lue);
	log_leveAdif
	if Gd>yd>kOog_levev.*	mmyv.-setyd;yvGd>Sengt@Gv>ngth =  cam hrt bw thread failed\n");
		/* No		/* MaIC\n");
		/* No		/yd>SCOMP_DEFAULT);
		mm_qto notifiVGd>wE2C>w	mm_qto notv4Gd>wE2C>w	mm_qto g notv4Gd>wE2C>w	mmfigtal_mix_li@ notv4Gd>wE2C>wVGtv4Gd>wW");
		/yv+g, "for (j = 0I2)[reqeC>yd>yd>Un2 larb, port, bw, old_larb_miy;fC>wt, bw, v>;);
	logC>yd>yd>yd>yd>g;_Gv%os_@;);
	logC>yd>ydV+ister_qos_notifier(&sy>AtalQ."oC>yd>gifier(&vGd>yd>yd>koC>k for (j = 0I2)[reqeC>yd>yd>UnC>gZE(mp_type = _DETECT
	regG_id]tMMMMMMMMMMMptFose_larb_mode, 04Gd#dt_req[req_id]d>yqos_dif
	if Gd>yd= _DETECd;.channel = MtDaram_opsequest(&cams_add_re;ISM
static int notify_bw_thro, bw, d;.chanC>yd>yd>yd>yh =  ttle");
}

static void del, p, value) {
		if;
s32 mm_hrt_rechannel = MtDaax_omode,
		MMPRax_o.chanC>yd>yd>yd>y_larbs & 1echannel = MtDaex(
				mmqos_,o.chanC>yd>yd>yd>yuf + lengVALUE;

	cam_occ_bw = draequelt * cam_occ_ratio());
	else
...
EXPOR);
	log_leveAdif
	if Gd>yd>kOog_levev.*	mmyvechannel = MtDad>Sengt@Gv>ngth =Gd>wE2C>wVGtv4Gd>wWad failed\n");
		/* No		/* MaIC\n");
		/* Noechannel = MtDaex&echannel = MtDad>Sead &echannel = MtDa *ptr, *tmp_str;
	char *log2C>wVGtv4Gd>wWad dump_g2C>lam_counoigtal_repart, equedecoupl_li@ notv4Gd>wernel_#ifd);
		/yv+g, "for (j = 0I2)[reqeC>yd>yd>Un2 larb, port, bw, old_lrt, i_freq;
	list_ler_block test_notifier = {
	.notiter_qnotice( bw_valt_ly>AtalQ."oC>yd>gif3er(&vGd>yd>yd>koC>k for (j = 0I2)[reqeC>yd>yd>UnC>gZE(mp_type = _DETECT
	regG_id]tMMMM
alue);
	}eq_lIZED_e_ostd = false;
			
writelevel & IZED_hrt_data -= req->hrt_value;
		ernel_;
	}
t_lomm][i]ue;
		=st_ler_blo	unsigned long gd>wernel_#ifd)larbs_info(log_str);
		tmp_str = log_str;
		if (otal,(len > 0) {
			{
		mmdvfs_get_larb_node(&pdev->de PORT_VIRTUAL_CCU_Crb_req[SMI_PMQOS_LAR"inva*req = NULL;
...
modr_notice("no larbs info to print\n");
		kfree(log_str)
static u3(char *buf, const struct kernel_param *kp)
{
	internel_#ifd>Sengt@Gv>ngth =Gd>wernel_#if,_bus_hang_deternel_#if,_* MaIC\n");
		/* Noernel_#if, kernel_#ifd>Se,_b(&ut_rptr, *tmp_str;
	char *loernel_#if,_bv4Gd>wEstatRT_VMAX_ paratic u*nb,
		un());(block *nb,
		un) MaIC\n")n 0;
ice *pdev)) MaIC\n")pr_no
	}

	pr_nic ump_str;r *lRIPernel"MTK;
	}

#ifdef " *tmp_str;AUTHOR("Damon Chu<damon.chu@		SMI_COi].c>" *tmp_str;LICENSE("GPL" i < SMI_#inclu_idsd>wux/ker>wW.h>_#inclu_idsd>wux/math64.h>_#inclu_id" - lengt	}

.h"dvfs current larb setti*move_k, 0xffff) << 16) |
d)
{
#ifd2 get_tothn_hrt_bp_larVGtvwner_l;
	}) : 1;
	if (hs_set_requeslarb_req[i].comm_port != SMI_,turn 0;
}

s32 get_virtual_ps32 ged>SCt_request(req,G_PULSE, larb_sofONE);
		mm_qos_updarn 0;
}

s32 get_virtual_ps32 geid,
		bw_vad * 3, 1);
		s32 gemp_value(enum_req->bw, larb_sofONE);
		mm_q->hrt_value;
			hrt_port = true;
		} else
			bw += get_comp_value(enum_req->bw, larb_sofONE);
		mm_ort) {
		req->ostd = SHIFT_ROUND(req->ostd * 3, 1);
		if (larb_req0, MAX_DUMP -w)
		pr_notice("mm_set=0x%08x bw=%u ostd=%u hrt=%u comp=%}_LARB_COUNT; i++) {
		larb_bw = (larb_req[i].comm_port != SMI_COMMMAX_DUMP -w)
		pr_notice("mm_set=2, smi_hrt_clk, 0xffff));
#endif
	}

	/}
owner_list, owner_node) {
		mm_qos_set_request(req,req->bw, larb_sofONE_only_hrt_bw - total_used_hrt_bw + src_hrt_bw;

	if (SMI_PMQOS_LAR, larb_sofONE_onl_RATIO(camera_max_bw)/cam_occ_ratio();
		if (cam_occ_max_bw > 0)
0, MAX_DUMP - lenget = false;
	pr_notice("set cam MAX_DUMP - length,
				"  %u", larb_req[arb_soft, qos_status);
#endif
	if (likely(force_larb_mlocking_notifier_ch