...
struct kbase_gator_hwcnt_handles *kbase_gator_hwcnt_init(struct kbase_gator_hwcnt_info *in_out_info)
{
	...
	/* If we are using any other device */
		 ...
		 * are always presmitedO3YO3H* T3Yk&=s3YO3YO3C
Limited. All rights reserved.
 *
 O3:H* Tr{Nc_bits = fls64(core_mask);
...
d. All rights rk&Tis program is subject to the termsO3YO3:am is k=NTis program is subject to the termsO3YRll_t-rSso. AlMU_L2_BLOCK;

			in_out_info->hwc_la].core_mask;

			 = MMU_L2_BLOCK;

			in_out_info->hwc_layout[i++] = RESERV/
	} else {>>= 1default:
			haCalculawareited  All muP_Kb	returswcns re;
	leited  All 	in_out_WARN_ON(ited. All rSs_hwcnt_ese_hwcnt_d_bytnt_>enable_map);
t_infor_alloc(metadata, &hn_oukbase_hBLOuf;

	in_out_inf All riited. Allreturode;
	s.jm_bm_BLOCKS_PER_GROUed.lse [re alway];turode;
	s.e_mas_bm_BLOCKS_PER_GROUed.lse [Limited. Al];turode;
	s.shadas_bm_BLOCKS_PER_GROUed.lse [].core_mask;];turode;
	s.mmuothebm_BLOCKS_PER_GROUed.lse [to the terms];tuux.h"
#include ta = kbase_hfrom_out_info(_metadata(
		handnabrode;
	setad		goto release_device;R_BLOCK			64
#defi_c;
	test device */
	hand->kbdev = kbaer_metadata(
		hander_metada MALIetadata, &hand->subjers_mali_evice */
	hanpu_pERV/"Fa_ma