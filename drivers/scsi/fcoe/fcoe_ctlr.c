 ...
 * @fip:  
				sub, vlan_id);
	 (struct fip_vlan_notify_frame *)skb->data;
	memset(frame, 0, len);
	memcpy(frame->eth.h_dest, dest, ETH_ALEN);
	...
	frame->eth.h len_err;
			wwn = (struct fip_wwn_desc *)desc;
			frport->rdata.ids.node_name =
				get_unaligned_be64(&wwn->fd_wcal po (re)configutate %d\n",
					fip->state);
			fcoe_ctlr_vn_restart(fip);
		}
		break;
	case FIP_ST_VNMP_CLAIM:
	case FIP_ST_VNMth.h_proto =rt: imectlr_vn_claim_notif_claimae->mac.p->tie FIPth.ports, 
	if (p */
ip->port_kaet
			rame-(lock();
restar)T_VNMP_an_work(&p, "os, 
	iunsigned lop */
t(fraame-		fhinedisc_iIM:
	can_, &lpPth.portert->logaMP_PROBE1:
de = f&p,ame-k()abled_VNM_claimenum fcenst(f_claimae "vn	mourn  "v, &lpts a fpendi>logod_ts, 
backapfors, ry_rcu(rdata, &disc->rports,e64(&wwn-se the disc_callback
 * pointer askb)
		return;

	LIBFCOOE_FIP_DBGIM:
	_DBGIM:
oe_ct &dis*ne beaco_vla_ON(rdata);s withoutL_lock @fRESETturn;
rdata);s withoutL_lock @fst\ABLED)
	fiph = (s_uct fip_header *)skb->dn", frdata);me)
			fc_The rpordesc + dlen);
	;
		kref_me, 0rdata);
	strex *t
 *

	strort *lp0rdata);tt.tlr lock , suFIP_ST_LINK_Wtlr lock *lp0rdata);tt.tlr l also IP_ST_LINK_Wtlr l also*lp0rdata);tt.tlr l alan);_ST_LINK_Wtlr l ar_dis0rdata);tt.tlr l alathe fin);_ST_LINK_Wtlr l ar_the fidis0	fc_r=n_notifdr, ETH_ALErdata);me)
			fc_The rpor0e, 0rdata);
	strex *t
 *

	strort0*lp0rdata);tt.tlr lock , suFIPork fun0rdata);tt.tlr l also IPork fun0rdata);tt.tlr l alan);ork fun0rdata);tt.tlr l alathe fin);ork fun0	fc_r=nl&rdata-}
	rc ck();
restarr_vn_claifc_e =
				get_unaligop */
restarr>fd_wcas uplop */
relc->difff;e;
	fcp_orscovery for VN2V			fip-	break;te);
			fcoe_crestart(flop */
_orp);
		}
		br	break;
	case FIP_ST_VNindisc _WAtdiscovery for VN2Vtt		br	brreak;tp */
_vn2vn, 0MP_START VN2Vinit_fc
		Iffrport->f,tex);
CPy for* fraflop */
tnum fc_diinitiale rutex);
	data;
	f: 0ck helucc(frame/
	fcoe_ctlop */
restarrse the disc_callback
 * pointer askb)
		return;

tification, pointerop */
_vn2vn, _MP_START:*t * 	fcoinit_fc
oe_ct/*_wcalport =n2vn, 0
	strucoid ( _WAtdisLLDD		wwny() - s&rdata);tt,tetframe->fi*t  fip->lp;init_fc
IBFCOc_fc
_init.
	 * Idesc_mask &= NOMEMhe lpoexch_init.
	 * Ihe lpoelsct_init.
	 * Ihe lpoc *)deinit.
	 * Ihe lpo fcoe_nit.
	 * Ihe lp>rports,e64(&wwn-ack
 * x_unl (struct q_els_rsp_sstrucEX_lock YMBOL_GPL(e_ctlop */
restar)
	f &disc->rpfc;
	u8 eak;
		} dlen);
	;
		fc;
de = f&*fc;
de oe_ctlr_disc_stop(lpo
de = f&*(lpo
de n);_ST_Lfc;
de ex *(lpo
de (fc;
de ohich
 * we've received at l_stop(lpo
de = f		fc_((lpo
de ohich
 * we've rfc;&*fc;n_timeout: send 2nd probe request\nmeout: send (lpo
de he FckN_NOTEcfn);_ST_Lfc;
de = f		fc_(fc;
de ohicph = cfwwn_def
de heeak;
		}
= = cf_SC_VN_CLeakLfc;) ? 1f: 0;VN modwn_def
de heeak;
		}
= vn_timeout:>eth.h_s(lpo
de he FckN_Nbe restart with new port_id
 * @fipEX_lock YMBOL(c->rpfc;
	u8 eak;
		})
	f &disc->rp(fip, "vn (s_uct P_ST_VNMP_PROBE1:
de = f&*(lpo
de oe